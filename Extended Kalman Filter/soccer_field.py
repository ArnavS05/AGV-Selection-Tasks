...
class Field:
    ...
    def G(self, x, u):
        ...
        '''
        G=[[dx_t/dx_t-1,     dx_t/dy_t-1,     dx_t/dtheta_t-1],
           [dy_t/dx_t-1,     dy_t/dy_t-1,     dy_t/dtheta_t-1],
           [dtheta_t/dx_t-1, dtheta_t/dy_t-1, dtheta_t/dtheta_t-1]]
        '''
    ...
    def V(self, x, u):
        ...
        # YOUR IMPLEMENTATION HERE
        '''
        V=[[dx_t/drot1,     dx_t/dtrans,     dx_t/drot2],
           [dy_t/drot1,     dy_t/dtrans,     dy_t/drot2],
           [dtheta_t/drot1, dtheta_t/dtrans, dtheta_t/drot2]]
        '''
    ...
    def H(self, x, marker_id):
        ...
        pred_x, pred_y, pred_theta = x.ravel()
        # YOUR IMPLEMENTATION HERE
        # The argument x of this function represents the predicted value of x
        # based only on dynamics and control.
        
        # Bearing is the angle between a line of inclination prev_theta and
        # a line joining the robot and the landmark.
        # Thus, if bearing=b, then b = atan2(dy, dx)-prev_theta
        # where dy and dx is the distance between landmark and robot, along
        # with correct sign (+/-) (so that angle with correct quadrant is received.
        '''
        H=[db/dpred_x, db/dpred_y, db/dpred_theta]
        '''
        dx = Field.MARKER_X_POS[marker_id]-pred_x
        dy = Field.MARKER_Y_POS[marker_id]-pred_y
        return np.resize(np.array([dy/(dx**2+dy**2), -dx/(dx**2+dy**2), -1]), (1,3))
